...
class MonsterDamage: Decodable {
    var id:UUID = UUID()
    var damageType: URL_WoTC?
    var damageDice: String?
    
    @Relationship(deleteRule: .cascade) var action: Action? // Single relationship
    @Relationship(deleteRule: .cascade) var legendaryAction: LegendaryAction? // Single relationship
